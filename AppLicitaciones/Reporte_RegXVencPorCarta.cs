...
{
    public partial class Reporte_RegXVencPorCarta : UserControl
    {
        MainConfig mc = new MainConfig();
        int idLicit = 0;
        public Reporte_RegXVencPorCarta()
        {
            InitializeComponent();
        ...
        private void Reporte_RegXVencPorCarta_Load(object sender, EventArgs e)
        {
            radAct.Checked = true;
        }
        ...
        private void cmbNumLicit_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            MostrarRegistrosXVencer(idlicit);
        }

        private void MostrarRegistrosXVencer(int idBases)
        {
            
            DateTime fechaOptima = DateTime.Today.AddDays(-150);
            ...
            this.tlvReg.ChildrenGetter = delegate (Object x)
            {
                ...
                if (x is VinculoRegistros)
                    return RegistroSanitario.GetRegistros().Where(y => y.Id == ((VinculoRegistros)x).Nombre && y.Vencimiento < fechaOptima);
                ...
                using (MemoryStream myMemoryStream = new MemoryStream())
                {
                    ...
                    foreach (Item item in c.ItemsPorLicitacion(idLicit))
                    {
                        ...
                        foreach (CucopVinculos cu in item.Vinculos)
                        {
                            ...
                            {
                                if (RegistroSanitario.GetRegistros().Where(x => x.Id == re.Nombre && x.Vencimiento < fechaOptima && !x.Prorrogas.Any()).Any())
                                {
                                    registros += "\n" + RegistroSanitario.GetRegistros().Where(x => x.Id == re.Nombre && x.Vencimiento < fechaOptima).Single().Nombre;
                                    vencimientos += "\n" + RegistroSanitario.GetRegistros().Where(x => x.Id == re.Nombre && x.Vencimiento < fechaOptima).Single().Vencimiento;
                    ...
                    string finaldest = svg.SelectedPath + @"\Reporte de Registros Por Vencer de " + c.Nombre + " en " + licit.NumeroLicitacion + ".pdf";
