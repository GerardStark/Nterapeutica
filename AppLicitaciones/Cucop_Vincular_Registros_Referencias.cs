...
{
    public partial class Cucop_Vincular_Registros_Referencias : Form
    {
        int id_registro = 0,  id_vinculacion = 0, filtro_flag = 0;
        MainConfig mc = new MainConfig();
        string ctrl = "", valor = "";
        List<Int32> claves = new List<int>();
        public Cucop_Vincular_Registros_Referencias()
        {
            InitializeComponent();
        ...
        private void btn_guardar_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                con.Open();
                SqlCommand cmdelete = new SqlCommand("DELETE FROM cucop_vinculos_registros_referencias Where id_vinculo_registro = " + id_vinculacion + "", con);
                ...
                claves.ForEach(delegate (int id)
                {
                    ...
                    try
                    {
                        SqlCommand cmd = new SqlCommand("INSERT INTO cucop_vinculos_registros_referencias (id_vinculo_registro,id_referencia,actualizado_en)" +
        ...
        private void btn_filtrar_Click(object sender, EventArgs e)
        {
            ...
            if (ctrl != "" && valor != "")
            {
                try
                {
                    ...
                    "from registros_claves_referencias where " + ctrl + " Like '%" + valor + "%'", con);
        ...
        public void mostrarReferenciasRegistro(int id_registro, int id_vinculacion)
        {
            ...
            try
            {
                ...
                {
                    DGV_Referencias.Rows.Add(dr.ItemArray);
                    marcarclaves(Convert.ToInt32(dr["id_clave_registro"]),id_vinculacion);
                }
                con.Close();
        ...
        private void marcarclaves(int id_clave,int id_vinculacion)
        {
            ...
            {
                SqlConnection con = new SqlConnection(mc.con);
                SqlCommand cmd = new SqlCommand("Select id_referencia from cucop_vinculos_registros_referencias where id_vinculo_registro = @idVinculo and id_referencia = @idref", con);
