        ...
        private void btn_imprimir_Click(object sender, EventArgs e)
        {
            ...
            if (checkBox1.Checked == true)
            {
                using (MemoryStream myMemoryStream = new MemoryStream())
                {
                    ...
                    myDocument.Open();
                    //creacion del header
                    Paragraph header = new Paragraph("Listado de registros sanitarios vencidos", times);
                    ...
                    table.AddCell(new Phrase("Denominacion Distintiva", times));
                    ...
                    var registros = (from rs in RegistroSanitario.GetRegistros()
                                    where rs.Vencimiento <= DateTime.Today
                                    select rs).ToList();
                    foreach (RegistroSanitario item in registros)
                    {


                        if (!item.Prorrogas.Any())
                        {
                            table.AddCell(new Phrase(item.Generica, times));
                            table.AddCell(new Phrase(item.Nombre, times));
                            table.AddCell(new Phrase(item.Vencimiento.ToString(), times));
                            table.AddCell(new Phrase(item.Titular, times));
                        }
                    ...
                    string finaldest = svg.SelectedPath + @"\Reporte de registros vencidos capturados.pdf";
